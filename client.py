    ...
    try:
        while detection_active:
            ...
            if "error" not in results:
                ...
                detection_results.append({"timestamp": timestamp, "results": results})
                ...
                # Print results
                print(
                    f"[{timestamp}] Classification: {highest_category[0]} ({highest_category[1]*100:.2f}%)"
                )

                # Check if it's a deepfake or AI-generated with high confidence
                if (
                    highest_category[0] == "Deepfake"
                    or highest_category[0] == "Artificial"
                ) and highest_category[1] > 0.7:
                    print(
                        f"⚠️ ALERT: Potential {highest_category[0]} detected with {highest_category[1]*100:.2f}% confidence!"
                    )
...
def start_detection(region=None, interval=1.0, face_only=True):
    ...
    capture_thread = threading.Thread(
        target=capture_screen, args=(region, interval, face_only)
    )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--region", type=str, help="Screen region to capture (left,top,right,bottom)"
    )
    parser.add_argument(
        "--interval", type=float, default=1.0, help="Capture interval in seconds"
    )
    parser.add_argument(
        "--no-face-crop", action="store_true", help="Disable face cropping"
    )
    parser.add_argument(
        "--server", type=str, help=f"Inference server URL (default: {server_url})"
    )
